...
{
    internal static class LocalV2ExceptionHandler
    {
        ...
        internal static string WriteLog(Exception exception, string context, string? logRoot = null)
        {
            ...
            string root = string.IsNullOrWhiteSpace(logRoot)
                ? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
                    "DanVille50", "Dan's RBI Baseball 2026", "Logs", "LocalV2")
        ...
        private static void HandleUiException(Exception exception)
        {
            ...
            try
            {
                ...
                DialogResult result = MessageBox.Show(
                    "Local Version 2.0 recovered from an unexpected error.\n\n" +
                    (exception?.Message ?? "Unknown error") + "\n\n" +
                    "Save the current dynasty under a new filename before continuing.\n\n" +
                    LogMessage(log) + "\n\nContinue running the application?",
                    "Local Version 2.0 Error",
        ...
        private static void HandleFatalException(Exception exception, string context)
        {
            ...
            try
            {
                ...
                MessageBox.Show(
                    "Local Version 2.0 encountered a fatal error and must close.\n\n" +
                    (exception?.Message ?? "Unknown error") + "\n\n" + LogMessage(log),
                    "Local Version 2.0 Fatal Error",
        ...
        private static string BuildLog(Exception exception, string context)
        {
            ...
            text.AppendLine("Dan's RBI Baseball 2026 - Local-only Version 2.0");
